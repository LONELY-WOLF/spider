...
USBH_Class_cb_TypeDef HID_cb = { USBH_HID_InterfaceInit, USBH_HID_InterfaceDeInit, USBH_HID_ClassRequest, USBH_HID_Handle };
...
static USBH_Status USBH_HID_InterfaceInit(USB_OTG_CORE_HANDLE *pdev, void *phost)
{
	...
	if (pphost->device_prop.Itf_Desc[0].bInterfaceSubClass == 0) // HID_BOOT_CODE)
	{
		...
		maxEP = ((pphost->device_prop.Itf_Desc[0].bNumEndpoints <= USBH_MAX_NUM_ENDPOINTS) ? pphost->device_prop.Itf_Desc[0].bNumEndpoints :
		...
		case HID_POLL:
			...
			if ((HCD_GetCurrentFrame(pdev) - HID_Machine.timer) >= HID_Machine.poll)
			{
				...
				if (HCD_GetURB_State(pdev, HID_Machine.hc_num_in) == URB_DONE)
				{
					...
					if (HCD_GetURB_State(pdev, HID_Machine.hc_num_in) == URB_STALL) /* IN Endpoint Stalled */
					{
...
static USBH_Status USBH_Get_HID_ReportDescriptor(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint16_t length)
{
	...
	USB_DESC_HID_REPORT, pdev->host.Rx_Buffer, length);
...
static USBH_Status USBH_Get_HID_Descriptor(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint16_t length)
{
	...
	status = USBH_GetDescriptor(pdev, phost,
	USB_REQ_RECIPIENT_INTERFACE | USB_REQ_TYPE_STANDARD,
	USB_DESC_HID, pdev->host.Rx_Buffer, length);
