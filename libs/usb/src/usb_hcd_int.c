									...
									if (hcint.b.datatglerr)
									{
										...
										if (hcint.b.chhltd)
										{
											...
											if (pdev->host.HC_Status[num] == HC_XFRC)
											{
												...
												if (pdev->host.HC_Status[num] == HC_NAK)
												{
													pdev->host.URB_State[num] = URB_NOTREADY;
												}
												else
													if (pdev->host.HC_Status[num] == HC_NYET)
													{
														...
														if (pdev->host.HC_Status[num] == HC_STALL)
														{
															pdev->host.URB_State[num] = URB_STALL;
														}
														else
															if (pdev->host.HC_Status[num] == HC_XACTERR)
															{
			...
			if (hcint.b.chhltd)
			{
				...
				if (pdev->host.HC_Status[num] == HC_XFRC)
				{
					...
					if (pdev->host.HC_Status[num] == HC_STALL)
					{
						...
						if ((pdev->host.HC_Status[num] == HC_XACTERR) || (pdev->host.HC_Status[num] == HC_DATATGLERR))
						{
							pdev->host.ErrCnt[num] = 0;
							pdev->host.URB_State[num] = URB_ERROR;
