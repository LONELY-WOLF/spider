...
void CAN_FilterInit(CAN_FilterInitTypeDef* CAN_FilterInitStruct)
{
	...
	if (CAN_FilterInitStruct->CAN_FilterScale == CAN_FilterScale_16bit)
	{
		...
		CAN1->sFilterRegister[CAN_FilterInitStruct->CAN_FilterNumber].FR1 = ((0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterMaskIdLow) << 16) | (0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterIdLow);
		
		/* Second 16-bit identifier and Second 16-bit mask */
		/* Or Third 16-bit identifier and Fourth 16-bit identifier */
		CAN1->sFilterRegister[CAN_FilterInitStruct->CAN_FilterNumber].FR2 = ((0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterMaskIdHigh) << 16) | (0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterIdHigh);
	}
	...
	if (CAN_FilterInitStruct->CAN_FilterScale == CAN_FilterScale_32bit)
	{
		...
		CAN1->sFilterRegister[CAN_FilterInitStruct->CAN_FilterNumber].FR1 = ((0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterIdHigh) << 16) | (0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterIdLow);
		/* 32-bit mask or Second 32-bit identifier */
		CAN1->sFilterRegister[CAN_FilterInitStruct->CAN_FilterNumber].FR2 = ((0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterMaskIdHigh) << 16) | (0x0000FFFF & (uint32_t) CAN_FilterInitStruct->CAN_FilterMaskIdLow);
	}
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef* CANx, uint8_t CAN_OperatingMode)
{
	...
	if (CAN_OperatingMode == CAN_OperatingMode_Initialization)
	{
		...
		if (CAN_OperatingMode == CAN_OperatingMode_Normal)
		{
			...
			if (CAN_OperatingMode == CAN_OperatingMode_Sleep)
			{
				/* Request Sleep mode */
...
FlagStatus CAN_GetFlagStatus(CAN_TypeDef* CANx, uint32_t CAN_FLAG)
{
	...
	if ((CAN_FLAG & CAN_FLAGS_ESR) != (uint32_t) RESET)
	{
		...
		if ((CAN_FLAG & CAN_FLAGS_MSR) != (uint32_t) RESET)
		{
			/* Check the status of the specified CAN flag */
			...
			if ((CAN_FLAG & CAN_FLAGS_TSR) != (uint32_t) RESET)
			{
				/* Check the status of the specified CAN flag */
...
void CAN_ClearFlag(CAN_TypeDef* CANx, uint32_t CAN_FLAG)
{
	...
	else /* MSR or TSR or RF0R or RF1R */
	{
		...
		if ((CAN_FLAG & CAN_FLAGS_RF0R) != (uint32_t) RESET)
		{
			...
			if ((CAN_FLAG & CAN_FLAGS_RF1R) != (uint32_t) RESET)
			{
				/* Receive Flags */
