...
void LTDC_Init(LTDC_InitTypeDef* LTDC_InitStruct)
{
	...
	LTDC->GCR |= (uint32_t) (LTDC_InitStruct->LTDC_HSPolarity | LTDC_InitStruct->LTDC_VSPolarity | LTDC_InitStruct->LTDC_DEPolarity | LTDC_InitStruct->LTDC_PCPolarity);
...
void LTDC_LayerPosition(LTDC_Layer_TypeDef* LTDC_Layerx, uint16_t OffsetX, uint16_t OffsetY)
{
	...
	if (tempreg == LTDC_Pixelformat_ARGB8888)
	{
		...
		if (tempreg == LTDC_Pixelformat_RGB888)
		{
			temp = 3;
		}
		else
			if ((tempreg == LTDC_Pixelformat_ARGB4444) || (tempreg == LTDC_Pixelformat_RGB565) || (tempreg == LTDC_Pixelformat_ARGB1555) || (tempreg == LTDC_Pixelformat_AL88))
			{
				temp = 2;
			}
			else
...
void LTDC_LayerPixelFormat(LTDC_Layer_TypeDef* LTDC_Layerx, uint32_t PixelFormat)
{
	...
	if (PixelFormat == LTDC_Pixelformat_ARGB8888)
	{
		...
		if (PixelFormat == LTDC_Pixelformat_RGB888)
		{
			temp = 3;
		}
		else
			if ((PixelFormat == LTDC_Pixelformat_ARGB4444) || (PixelFormat == LTDC_Pixelformat_RGB565) || (PixelFormat == LTDC_Pixelformat_ARGB1555) || (PixelFormat == LTDC_Pixelformat_AL88))
			{
				temp = 2;
			}
			else
